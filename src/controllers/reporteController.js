// ===============================================================
//   REPORTE PDF AETECH – VERSIÓN FINAL ESTABLE (FOOTER PERFECTO)
// ===============================================================
...
function encabezado(doc, logoBuf, watermarkBuf) {
  ...
  // Título
  doc.fontSize(24).fillColor("#004b85").text("AE TECH", 140, 28);
  doc.fontSize(11).fillColor("#444").text("Reporte oficial de servicio", 140, 58);

  // Línea divisora
  doc.moveTo(40, 90).lineTo(doc.page.width - 40, 90).stroke("#CFCFCF");

  doc.moveDown(2);
}

// =========================================================
//   Footer PRO (SIEMPRE ABAJO)
// =========================================================
function footer(doc) {
...
// =========================================================
//   GENERAR REPORTE PDF – SIN PÁGINAS EN BLANCO
// =========================================================
exports.generateReportePDF = async (req, res) => {
  ...
  try {
    ...
    // ============================================
    // FOOTER Y ENCABEZADO automáticos por página
    // ============================================
    doc.on("pageAdded", () => {
    ...
    footer(doc); // Footer de la primera página
    ...
    for (const ev of evidencias) {
      const imgBuffer = await procesarImagen(
        ev.archivoUrl,
        MAX_W,
        MAX_H
      );
      ...
      if (y + img.height > doc.page.height - 160) {
        doc.addPage();
        y = 140;
      }

      doc.image(imgBuffer, x, y, { width: img.width });
      doc.fontSize(12).text(ev.titulo || "Evidencia", x, y + img.height + 4);
    ...
    if (evFirma) {
      ...
      if (firmaBuf) {
        ...
        doc
          .fillColor("red")
          .text("⚠ No se pudo cargar la firma.");
      }
    }
