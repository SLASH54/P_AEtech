...
async function deleteData(endpoint) {
 ...
 // e. Agregar el botón de PDF individual dentro de la tarjeta
            htmlContent += `
                <div style="text-align: right; padding-top: 10px; border-top: 1px dashed #eee; margin-top: 10px;">
                    <button class="pdf-individual-button" onclick="exportarEvidencia('${actividadId}')">
                        📄 Imprimir PDF (Actividad #${actividadCounter})
                    </button>
                </div>
            `;
        ...
        // =======================================================
        // LÓGICA DE GENERACIÓN DE PDF (INDIVIDUAL) - FIX CRÍTICO
        // =======================================================
        async function exportarEvidencia(actividadId) {
            const element = document.getElementById(actividadId);

            if (!element) {
                console.error(`Error: Elemento con ID ${actividadId} no encontrado.`);
                return;
            }
            
            // 1. Ocultar el botón de la vista principal temporalmente
            const button = element.querySelector('.pdf-individual-button');
            if (button) button.style.display = 'none';

            // 2. CREAR UN CONTENEDOR TEMPORAL Y LIMPIO PARA EL PDF
            const pdfWrapper = document.createElement('div');
            // Aplicar estilos críticos para la captura (fondo blanco y ancho definido)
            pdfWrapper.classList.add('pdf-content-wrapper'); 
            
            // Clonar el contenido de la actividad
            const contentToClone = element.cloneNode(true);
            const clonedButton = contentToClone.querySelector('.pdf-individual-button');
            if (clonedButton) clonedButton.remove();

            // Insertar el contenido clonado
            pdfWrapper.appendChild(contentToClone);

            // Agregar el mensaje de agradecimiento de AE Tech al final
            const footerHTML = `
                <div class="pdf-footer">
                    <h5>¡Gracias por su confianza!</h5>
                    <p>Documento de evidencia generado por el sistema AE Tech.</p>
                    <p>Fecha de impresión: ${new Date().toLocaleDateString('es-ES')}</p>
                </div>
            `;
            pdfWrapper.insertAdjacentHTML('beforeend', footerHTML);

            // 3. **FIX CRÍTICO:** Insertar el contenedor temporal en el DOM. 
            // Esto es vital para que html2canvas pueda renderizar las imágenes Base64.
            document.body.appendChild(pdfWrapper);
            
            // 4. Obtener el número de actividad para el nombre del archivo
            const titleElement = element.querySelector('h4');
            const activityNumberMatch = titleElement ? titleElement.textContent.match(/Actividad #(\d+)/) : null;
            const activityNumber = activityNumberMatch ? activityNumberMatch[1] : 'Individual';

            const today = new Date().toISOString().slice(0, 10); // Formato YYYY-MM-DD
            const filename = `Evidencia_Actividad_${activityNumber}_${today}.pdf`;
            ...
            const options = {
                ...
                html2canvas: { 
                    ...
                    logging: false, 
                    useCORS: true, 
                    allowTaint: true // Permitir que las imágenes se carguen sin errores CORS
                }, 
                jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
            };

            try {
                // 6. Ejecutar la conversión
                await html2pdf().from(pdfWrapper).set(options).save();
            } catch (error) {
                console.error("Error al generar el PDF:", error);
            } finally {
                // 7. Limpiar y Restaurar: Eliminar el contenedor temporal del DOM y restaurar el botón.
